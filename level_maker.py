...
for x in range(TILE_TYPES):
    if x >= 10:
        img = pygame.image.load(f'tiles/ground blocks/ground_{x}.png').convert_alpha()
    else:
        img = pygame.image.load(f'tiles/ground blocks/ground_0{x}.png').convert_alpha()
    img = pygame.transform.scale(img, (TILE_SIZE, TILE_SIZE))
    img_list.append(img)

save_img = pygame.image.load('uibuttons/ui_1.png').convert_alpha()
load_img = pygame.image.load('uibuttons/ui_0.png').convert_alpha()
...
while run:
    ...
    draw_text('Press UP or DOWN to change map', font, WHITE, 10, SCREEN_HEIGHT + LOWER_MARGIN - 60)

    # save and load data
    if save_button.draw(screen):
        # save level data
        with open(f'map{level}_data.csv', 'w', newline='') as csvfile:
    ...
    if load_button.draw(screen):
        ...
        scroll[0] = 0
        scroll[1] = 0
        with open(f'map{level}_data.csv', newline='') as csvfile:
